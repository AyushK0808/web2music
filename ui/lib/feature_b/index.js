...
export function registerFeatureBListener() {
  chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    ...
    // NOTE: currentWindow:true (rather than lastFocusedWindow:true) silently
    // returns an EMPTY array when queried from a service worker -- MV3
    // service workers have no "current window" concept at all (unlike MV2
    // background pages), so this previously dropped every single
    // FEATURE_A_HANDOFF with no error: activeTabId came back undefined,
    // the guard below failed, and runFeatureB() never ran.
