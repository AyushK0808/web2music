...
**Medium `[M]`**
- [ ] **Boilerplate stripping uses substring matching** — [Textextractor.js:111-114](data-extraction/Textextractor.js#L111-L114): `[class*="ad"]` deletes `shadow`/`gradient`/`download`/`badge`/`loading`; `menu` matches `document-menu` content wrappers. Tokenize className/id and match whole words. — **Vedant** (reassigned from Pari)
...
- [x] ✅ **PR #8** — **Pin `requirements.txt`**; add Pydantic request model (README already plans it); note `/generate` has no auth/rate-limiting (each request = GPU-minutes — cost-DoS surface; one sentence in deployment discussion). — **Tvisha** — ⚠️ **[PR #8](https://github.com/AyushK0808/web2music/pull/8)** (open; supersedes [PR #6](https://github.com/AyushK0808/web2music/pull/6)/[#7](https://github.com/AyushK0808/web2music/pull/7), which contain strict subsets of #8's commits — merge #8 only) has both pieces code-complete: `requirements.txt` pins to real, currently-published versions (verified against PyPI); `models.py` adds `MusicProfile`/`HandoffPayload` Pydantic models, `d1_validate.py` and `main.py`'s `/generate` now use them. **Not yet mergeable**: `MusicProfile.bpm` is `Field(ge=40, le=200)`, but B3's own bpm formula can legitimately emit **bpm as low as 25** for `sad` mood + `educational` page + `late-night` (verified against [b3_musicProfileGenerator.js:186-189](mood-classification/feature_b/b3_musicProfileGenerator.js#L186-L189)'s `clamp(raw, lo-20, hi+20)` with `sad`'s `bpmRange:[45,65]`) — a real request would get 422-rejected. Lower the floor or tighten B3's clamp before merging. Auth/rate-limiting note is still unaddressed. No automated tests added for the new models (PR description lists 4 manually-run cases only).
...
**One-sentence summary:** the existing findings list is accurate but is mostly Phase-1/2 hygiene; the paper lives or dies on Phases 3–4, and none of it can start until the B→D handoff connects (✅ mostly, PR #3 — `duration` + B-side contract still open), the valence sign is fixed (✅ PR #1, X3), and the loop detector stops being a no-op (✅ PR #9, X2 — reviewed, recommended for merge; also unblocks PR #4's fallback clips). **All three show-stoppers now have a reviewed fix in flight** — the critical-path gate has shifted from "does a fix exist" to "merge the open PRs (#1/#3/#4/#5/#8/#9/#10) into `main` and run the one end-to-end integration test (§5)." If the fine-tuning track (§4.4) is taken, the audio contribution stands on §4.4's control-fidelity results; if not, D remains substrate and the paper rests on Phases 3–4 as written. *(Note: PRs #1, #3, #4, #5, #8, #9, #10 are all still open — the ✅ marks describe their branches, not `main`.)*
