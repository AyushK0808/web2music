...
> **Updated: 2026-07-20 · ALL open PRs are now MERGED into `main`.** Verified against the merge history + the working tree, not the PR conversations: **#1** (Sneha, X3 + tier-2 hardening + Groq switch), **#2** (Pari, Feature A text-extractor cleanups), **#4** (Tvisha, retry + fallback logic — *includes PR #3's D-side handoff/prompt/instrument-map changes*), **#5** (Sneha, message-port/version/sensitive/`runB3` fixes), **#8** (Tvisha, Pydantic models + `duration_seconds` + `bpm ge=20` + `torch.compile` + pinned `requirements.txt`; **supersedes #6/#7**), **#9** (Vedant, X2 loop detection + equal-power crossfade), **#10** (Sneha, MV3 `tabState` + idle-fade + golden fixtures + silence-by-default). **The ✅ marks below now describe `main`, not just branches.** Consequently the critical-path gate from the 07-18/07-19 notes — "merge the open PRs" — is **cleared**; what remains is the §5 end-to-end integration test plus the still-open items below (none of which was ever in a merged PR). Re-verified against code as still open: X1's camelCase↔snake_case handoff leak (`contentCategory`/`atmosphereTags`/`listeningContext`/`timeOfDay`/`sensitiveOverride` — B4 emits camelCase, D's Pydantic `MusicProfile` reads snake_case → silently default; [b4_promptEngineer.js:208-231](mood-classification/feature_b/b4_promptEngineer.js#L208-L231) vs [models.py:11-24](audio-generation/models.py#L11-L24)); no `arousal` field anywhere in B; `fallback_clips/` + `generate_fallbacks.py` still absent (fallback code is live but has no clips to serve); the four `experiments/*.py` stubs + `research_log.md` still empty; the dead `< 1000 ms` guard still at [d4_process.py:48-49](audio-generation/d4_process.py#L48-L49); PR #9's synthetic loop checks still not committed as a test; and `data-extraction` still has no `node:test` suite. `loop_point_ms` **is** now surfaced in `/generate` metadata ([main.py:123](audio-generation/main.py#L123)) — that item is done. **See [ROADMAP_20-07.md](ROADMAP_20-07.md) for the done/to-do split per person.**
>
...
These three bugs mean the system a study participant would experience today is a **non-adaptive, non-looping, constant calm clip**. Any data collected before fixing them measures a system that doesn't exist. *(**All three show-stoppers are now fixed and merged into `main`** as of 2026-07-20: X1 largely resolved on the D side via PR #4 (which carried PR #3's changes) — the camelCase↔snake_case leak + missing `arousal` remain the open remnant; X3 fixed by PR #1; X2 fixed by PR #9. The X2 merge also unblocked PR #4's fallback-clip generation — though the clips themselves still don't exist (`fallback_clips/` empty, `generate_fallbacks.py` absent), so that follow-up is still open.)*
...
- [x] ✅ **Surface `loop_point_ms` in the `/generate` response metadata** so Feature C's Web Audio player can set `loopStart`/`loopEnd` from it — done: `loop_point_ms` is returned in the metadata block ([main.py:123](audio-generation/main.py#L123)). Pairs with the gapless-export item below (still open) and the round-trip test (still open). — **Vedant**
...
**One-sentence summary:** the existing findings list is accurate but is mostly Phase-1/2 hygiene; the paper lives or dies on Phases 3–4, and Phase 1's correctness gate is now largely met — the B→D handoff connects (✅ PR #4/#3, D side; B-side contract + `arousal` still open), the valence sign is fixed (✅ PR #1, X3), and the loop detector works (✅ PR #9, X2). **All three show-stoppers are now merged into `main`** — the critical-path gate has shifted from "merge the open PRs" (done) to "run the one end-to-end integration test (§5), then start Phase 2/3." If the fine-tuning track (§4.4) is taken, the audio contribution stands on §4.4's control-fidelity results; if not, D remains substrate and the paper rests on Phases 3–4 as written. *(Note: PRs #1, #2, #4, #5, #8, #9, #10 are all **merged** — the ✅ marks now describe `main`. #6/#7 were superseded by #8; PR #3's D-side changes landed via PR #4. The remaining unchecked boxes below were never in a merged PR.)*
