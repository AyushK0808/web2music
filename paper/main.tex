...
(Table \ref{tab:crossfade-sweep}, 50 to 500 ms) is a modest lever on
energy discontinuity, monotonically so (3.46 to 2.90 dB); the accompanying
spectral-centroid column is not monotonic (577, 343, 125, then back up to
195 Hz at 500 ms) -- unexplained at this $n=10$ secondary sample, and a
reason to check a larger one before treating 500 ms as strictly better
than 250 ms on both metrics at once.

\textbf{Selecting for measured seam, not chroma similarity, is the
largest lever in this paper.} Replacing the selection rule outperforms
widening the crossfade by an order of magnitude: minimising \emph{measured}
seam rather than trusting chroma similarity moves the loop point on 9 of
10 clips and cuts discontinuity from 3.46 to 0.33 dB, against widening's
3.46 to 2.90 dB above ($n=10$ secondary sample in both cases -- see the
gating note below on why this has not been re-run at the full 141-clip
scale).
...
is inadequate; the remedy is a classifier, not more keywords — the
deployment-level gap noted earlier, invisible to any evaluation testing
only the mechanism.
