...
\begin{figure}[t]
\centerline{\includegraphics[width=\columnwidth]{figures/pipeline.pdf}}
...
A request is first validated by the generation service, then looked up in a
Postgres cache indexed on key, style, mood and energy, and on duration,
tempo and arousal per bucket, excluding the seed. If a cache hit occurs, the
value at that key and the loop position is returned. If a cache miss occurs,
the clip is generated, post-processed and stored in object storage, and the
cache row is upserted. If any of these steps fail, a pre-generated clip for
that mood is used. In production, clips are batched by a single worker. Prior
to usage, a grid of ninety-nine combinations is pre-populated into the cache.
The five-stage pipeline is shown in Fig.~\ref{fig:featured}.
...
\textbf{Loop synthesis.} A one-off clip is generated by the synthesiser, not
a looped clip. Chroma features are used to create a self-similarity curve
\cite{foote} for potential loop candidates of length at least three seconds.
The best loop is selected and aligned to a bar using beat times
\cite{ellis, librosa}, assuming 4/4. The loop is then truncated at this
position, and the start and end of the loop are blended using a 250 ms
equal-power crossfade. The volume is normalised to $-18$ LUFS \cite{ebu128}
and the clip is exported as Ogg/Opus \cite{opus} instead of MP3. The
crossfade ensures the loop is continuous in energy across the seam; the
container, rather than the crossfade, is what makes it gapless. If MP3 is
used, an audible fill is added to each loop iteration by the encoding
process. A seam discontinuity is also computed as the evaluation metric in
\S\ref{sec:res-loop}.

\subsection{Playback and extension shell}

During playback a loop point is passed to two alternating decks of raw audio
buffers. These decks are then passed through a series of effects and gain
stages. This creates a separation between when the clip is actually heard and
when the clip is generated. It is achieved by having the client request a
pre-generated clip for a fallback mood in parallel with a generation. The
fallback clip starts immediately, and the generated clip is then crossfaded
in when the generation completes. An abort controller is used to allow a new
mood to cancel an in-progress generation.

\textbf{Restraint.} Sensitive pages are silenced prior to both hosted steps
if they are detected. An audible active tab is muted, while a background tab
is ducked. URLs that are internal to the browser, banking or payments are
also blocked. Telemetry is a local-only ring buffer, hashed and never
transmitted unless explicitly exported.
...
The evaluation has been structured into five different studies: a systems
benchmark, a cascade ablation, an assessment of audio quality and loop
quality, a perceptual study conducted on subjects who listened to generated
samples of traffic versus recorded traffic, and finally an in-the-wild
deployment study. Here we report results for the systems benchmark, the
cascade ablation, the objective portion of the audio and loop quality study,
the baseline for retrieval, and the restraint audit; however, we have not yet
received approval to conduct our perceptual study and our in-the-wild study,
because both require exposing human participants to the generated audio
versus simply playing back recorded or synthetically generated traffic.

\textbf{Stubs.} Extraction (Table~\ref{tab:extraction}) stubs inferencing
embedded into extraction; client latency (Table~\ref{tab:e2e-latency}) stubs
the generation service and classify proxy. Each number is generated by a
single named script against a metric registry that rejects unknown metrics.

\textbf{Corpus.} The cascade-ablation corpus contains 260 pages divided into
seven slices (182 general, 15 vocabulary mismatch, 15 non-English, 15 mixed
genre, 12 sensitive, 15 near miss, 6 bypass). There are three other derived
counts which do not use common arithmetic: the extraction snapshot collected
data on 255 of 260 pages; the ablation snapshot collected scores on 254 of
260 with category labels; and the annotator-majority re-scoring
(\S\ref{sec:res-classification}) was used on the 208 of 260 pages where there
was some overlap and the vote resolved.

\textbf{Adversarial slice.} The restraint audit also includes a separate
47-page slice (29 sensitive, 18 not) written by hand using terms found in the
detector's term list -- this represents white-box testing, but it is not
representative of a naturalistic sample.

\textbf{Annotators.} Only three of the authors were involved in labelling,
and they all labelled independently under a preference framing without
participating in a calibration round.

\textbf{Hardware.} All measurements were made on one CPU machine except for
the cache-cold generation measurements (Table~\ref{tab:d-latency-cold}),
which were performed on a separate GPU-hosted instance (Apple M4) via
tunnel.

\subsection{Annotation agreement and human ceiling}
\label{sec:res-agreement}

255 pages were annotated with category (13-way), mood (11-way) and
sensitivity (binary) by three annotators. Table~\ref{tab:agreement} shows
Krippendorff's $\alpha$ and the ceiling using leave-one-annotator-out. A
disagreement between two annotators is a tie, and is scored as an error when
...
For category and mood the ceiling sits below $\alpha$; for binary sensitivity
it sits above. Ties cannot occur there, and the 87.6\% label skew affects raw
agreement but not chance-corrected agreement. All three $\alpha$ values fall
below the minimum value of 0.667. For mood $\alpha = 0.195$, which is close
to chance. Using Russell's circumplex \cite{russell} and NRC norms
\cite{nrcvad}, $\alpha$ values of 0.121 and 0.300 are found. Coarsening does
not appear to offer any significant benefit. This suggests that mood is a
stylistic choice rather than a true mood. In this paper, accuracy in mood
detection is not measured; only category is reported in
Table~\ref{tab:tiers}. However, mood does have an effect on the music.

\subsection{Extraction cost}
\label{sec:res-extraction}

Descriptors were created using 105 real websites using a Chromium instance
...
Table~\ref{tab:extraction} shows the time taken to create a descriptor
compared to navigation time (1989 ms). The median time to create a descriptor
is 86 ms. The p95 value is 15 times the median and is caused by the search
within the vector store. 80.8\% of visible text was preserved during
boilerplate removal. When the embedding is stubbed, 86 ms represents a lower
bound. A related extraction metric is reported in
Table~\ref{tab:e2e-latency} and has a value of 464 ms. However, that metric
measures the cumulative data collected across multiple extractions run on
synthetic test content; a single traversal of a live website is not measured.
Thus the two values are not comparable.

\subsection{Which signals earn their cost}
\label{sec:res-signal-ablation}

The use of an offline ablation over 255 pages tests each signal separately by
replaying the keyword stage without one of them. Category never changes, since
it is text-only by construction. When colour is removed to test mood, this
causes a label to be changed on approximately 7.5\% of the pages, compared
with the 10.2\% of the method that uses only text. Behaviour and the embedding
are both always 0 by design -- the synthetic behaviour profile is fixed within
the corpus, and the embedding does not influence keyword-stage mood --
therefore there is no actual testing being done for these two signals.

\subsection{Generation latency and its failure modes}
\label{sec:res-latency}

Five different types of requests were made against the service.
...
Table~\ref{tab:d-latency} reports on a total of 144 of those requests, which
means using a mean would provide information about none of the things that
have actually occurred. The fallback clip is served in a median of 7 ms.
Generation completed on only three of the requests (82 s, 135 s and 203 s,
for 10 s, 15 s and 20 s clips). Three additional requests failed over to the
fallback after a median of 127 s. The twelve timeouts were all the result of
the first request for a 28-second clip of an uncached mood. Since the
service's single batching worker queues subsequent requests behind the
initial request that had not yet completed, subsequent requests for the same
mood did not encounter a cache hit until the server had completed generating
the response, and the client timed out. Fig.~\ref{fig:f1} shows the timeline,
Fig.~\ref{fig:f2} generation scaling with duration, and Fig.~\ref{fig:f3} the
...
These 28-second generations are censored at 300 s rather than being early
failures: at the real-time factor shown in Fig.~\ref{fig:f2}, a 28-second
clip sits at 230--280 s prior to post-processing, or at least 139 s longer
than some of the shorter clips, which is the minimum cost to generate a
28-second clip cold.

\textbf{Cache path.} A 2.08 s cache check is incurred every time a hit
occurs, and similar cost savings point towards storage I/O rather than cache
logic; cache-hit throughput scaled nearly linearly up to eight concurrent
requests ($7.94\times$). Fallback-then-swap is what makes the system usable
on CPU at all; its cost is that the first seconds -- and, on a timed-out
request, every second -- are generic.
...
The same single-mood sweep, re-run cache-cold on the GPU-hosted instance
(Table~\ref{tab:d-latency-cold}), puts median wall clock at 72 s, roughly a
third of the CPU figure.

\subsection{Client-side (in-browser) latency}
\label{sec:res-e2e-latency}

Eleven synthetic fixture pages are driven by the built extension using the
generation service and classify proxy stubs, with an end-to-end Playwright
harness for the built extension. 54 of 55 page loads produced audible output
($n = 54$).
...
From navigation to audible content, both the confidence window and the
fallback mechanism occur within one second. This meets the latency
requirement in \S\ref{sec:gaps}. The latency experienced by the user is
dominated by the window selected rather than by the model. One page is an
anomalous repeatable case (11.2--11.3 s); this occurs because the fixture
requires five extraction attempts prior to mood commitment. This is only for
the shell, and not the actual escalation rate over the real corpus.

\subsection{Cost and cache behaviour}
\label{sec:res-cost}

The escalation rate over the real corpus exceeds the keyword phase for 0.762
of the pages (an equivalent value was calculated in three different snapshots
of the corpus and yielded values of 0.756, 0.760 and 0.762); the value of
0.762 is reported consistently below. For an initial 18-page smoke test,
0.222 was observed. At this rate the cascade has a cost of 0.0132 USD per
thousand pages, which is insignificant in either direction. The purpose of
the cascade is not cost reduction, but rather to mitigate the problem of
disclosure. The cache hit rate is simulated and not observed: for a uniform
mood distribution the cache hit rate is 57 percent, and for a skewed mood
distribution it is 83 percent.
...
\midrule
A1 keyword only & .189 [.146,.236] & .315 [.246,.369] & 0 & 0 \\
A2 zero-shot only, ungated & .520 [.461,.583] & .562 [.494,.616] & 0 & .938 \\
A3 LLM only & .685 [.626,.744] & .678 [.620,.726] & 1.00 & 0 \\
A4 keyword$\to$LLM (prior) & .669 [.610,.724] & .677 [.616,.722] & .762 & 0 \\
A5 keyword$\to$ZS$\to$LLM (ours) & .622 [.559,.681] & .656 [.592,.704] & \textbf{.250} & .512 \\
\bottomrule
\end{tabular}
\end{center}
\end{table}
...
diagnostics threaded through \texttt{resolveSensitivity} itself (a
\texttt{zeroShotReason} distinguishing a confident verdict from an
abstention from the seven ways the tier never gets a real answer at all)
to distinguish a page the model actually classified from one that
silently fell back to the keyword verdict on a proxy timeout, and
\textbf{the result is a trade, not the repair the design verification
implied.} All three runs agree to six decimal places on every score for
every page that reached the model -- the entailment model itself is
fully deterministic; the instability seen in earlier, undiagnosed
attempts was network reliability (which pages round-tripped versus
timed out), not model noise. The stable result: false-negative rate
falls from 0.655 to \textbf{0.000} (29/29 genuine crisis pages now
...
detector, not better. All 18 non-sensitive pages reached the model; none
fell back (the only pages that do are hard-severe crisis pages the
design intentionally never escalates). Fourteen received a confident
verdict, ten of them wrong, including every one of the 6 purely benign
pages (recipe, programming, sport). The remaining four abstained rather
than answering confidently, and abstention cuts both ways: three
preserved an already-correct keyword verdict, but the fourth,
\emph{near-grief-degree}, did not. The keyword tier had already flagged
it, wrongly, on an ambiguous-term hit before the entailment tier was
ever consulted; abstaining meant the entailment tier never got a chance
to override that call, so a pre-existing keyword-tier error passed
through unchallenged rather than being independently reintroduced. This
is a third, genuinely distinct failure mode from the confident
misclassifications above, not the same one under another name --
whether it counts as a defect \emph{introduced} by adding the entailment
tier is a real question, since the same page was already wrong before
this fix existed either.
